...
# 4. Keep only Adelie and Gentoo penguins, then count the number

# of observations for each species and sex.
...
# Explicitly specify carrier as the join key using join_by().

#

# Save the result as flights_airlines.

# 8. Add the destination airport name to flights_small.
...
# Create a small dataset containing messy character strings.
...
# 16. Create a date from the separate year, month, and day

# variables below using make_date().

#

# Save the new variable as sample_date.
...
# 19. Create the following character vector:
...
# Convert size_class to a factor with the levels in this order:
...
# Then use levels() to check the order.
